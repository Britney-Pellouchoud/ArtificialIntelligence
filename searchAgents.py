...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        #print("*********************HITS HERE + " + str(state))
        """
        Returns whether this search state is a goal state of the problem.
        """
        "*** YOUR CODE HERE ***"
        
        #print("THIS IS STATE " + str(state[1]))
        count = 0
        for s in state[1]:
            if s == True:
                count += 1
...
def cornersHeuristic(state, problem):
    ...
    for c in cornerstate:
        if c == False:
            ...
            dist = manhatDist(corner, state[0])
            listLens.append(dist)
        count += 1
    if len(listLens) != 0:
        maxlen = max(listLens)  
    else:
    ...
    return maxlen # Default to trivial solution

    #closest corner that is open

    "*** YOUR CODE HERE ***"
    return 0 # Default to trivial solution

def manhatDist(firstpt, secondpt):
    x1 = firstpt[0]
    y1 = firstpt[1]
    x2 = secondpt[0]
    y2 = secondpt[1]
