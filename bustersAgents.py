...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            closest = ghost.argMax()
            distFromGhost = self.distancer.getDistance(closest, pacmanPosition)
            if distFromGhost < closestGhostDist:
                closestGhostDist = distFromGhost
                closestGhostPos = closest

        bestMove = None
        bestMoveDist = float('inf')

        for move in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, move)
            dist = self.distancer.getDistance(closestGhostPos, successorPosition)
